...
MANUAL_SECTIONS = [
    ("Getting Started", [
        ...
        ("table_row", "docflix-rename|Media Renamer"),
        ("table_row", "docflix-media|Media Processor"),
        ("table_row", "docflix-scale|Media Rescale"),
    ]),
    ...
    ("Main Window", [
        ...
        ("table_row", "Tools|Play, Media Details, Test Encode, Open Output Folder, Subtitle Editor, Batch Filter Subtitles, Media Processor, Audio Tools, Media Renamer, Media Rescale, Whisper Transcriber, Sub Extractor, Trailer Grabber"),
    ...
    ("Metadata & Tagging", [
        ...
        ("p", ""),
        ("h4", "Plex Filename Tag"),
        ("p", "When \"Plex\" is checked, the output filename includes the edition in braces and uses a clean filename without the encoding suffix:"),
        ("p", "Example: Superman {edition-Director's Cut}.mkv"),
        ("p", "In the Media Processor, in-place mode renames the file to include the edition tag and removes the original."),
        ("p", ""),
    ...
    ("Subtitles", [
        ...
        ("p", ""),
        ("h3", "Batch Filter"),
        ("p", "Tools \u2192 Batch Filter Subtitles. Process multiple subtitle files at once with filter checkboxes and batch Search & Replace."),
        ("bullet", "Drag and drop files or whole folders onto the list"),
        ("bullet", "Select All / Deselect All for the filter checkboxes"),
        ("bullet", "Output: overwrite the originals, or write to a named subfolder"),
        ("bullet", "Remembers your last selections \u2014 filters, search & replace, output mode and subfolder name are restored next time you open it"),
        ("p", ""),
        ("h3", "Forced Subtitle Editor"),
        ("p", "Build a FORCED subtitle track for mixed-language media \u2014 a track shown by default that covers only the foreign-language dialogue, while the main track carries everything."),
        ("note", "Opened from the Subtitle Editor: View \u2192 Forced Subtitle Editor. It is not in the main window's Tools menu, because it belongs to the subtitle editing workflow rather than the encode queue."),
        ("bullet", "Mark foreign-speech spans by ear on a waveform \u2014 A sets the start, B the end, Enter accepts the span"),
        ("bullet", "Drag span edges to fine-tune, delete anything marked in error"),
        ("bullet", "Edit the text of any accepted span by hand"),
        ("bullet", "Translation runs on accepted spans ONLY, never on raw detections"),
        ("bullet", "Mux the finished track into the video, set as default/forced"),
        ("p", ""),
        ("p", "An optional automatic language scan can PROPOSE spans for you, but it never decides. A detector cannot reliably tell a foreign-language interview from a suspenseful music cue \u2014 in testing it labelled a film score as Norwegian at 87% confidence. Every proposal is audited by ear before anything is translated."),
    ]),
    ...
    ("Tools", [
        ...
        ("bullet", "Real-time per-file progress bar"),
        ("bullet", "Output: replace in-place or save to folder"),
        ("bullet", "Per-file overrides via right-click"),
        ("p", ""),
        ("h3", "Audio Tools"),
        ("p", "Analyze-then-fix audio (Tools \u2192 Docflix Audio Tools). Measures each file and applies ONLY what is actually broken, so it is safe to run across a whole library. Video is always stream-copied \u2014 never re-encoded."),
        ("table_header", "Mode|What it does"),
        ("table_row", "Fix|Auto-repairs what it finds: dead or badly imbalanced channel, too-quiet audio, low-bitrate muffle"),
        ("table_row", "Normalize|Forces a whole folder to a uniform loudness target (two-pass loudnorm)"),
        ("p", ""),
        ("bullet", "Codec and bitrate dropdowns \u2014 AAC, AC3, EAC3, MP3, Opus, FLAC"),
        ("bullet", "Optional presence lift and denoise"),
        ("bullet", "Optional downmix to stereo"),
        ("bullet", "Downconvert Atmos / TrueHD / DTS-HD to AC3 (5.1) or AAC \u2014 objects flatten to the bed"),
        ("bullet", "Output: blank folder = an '_audio_fixed' folder beside each source. Originals are never touched."),
        ("bullet", "Remembers its settings between sessions"),
        ("p", ""),
        ("h3", "Whisper Transcriber"),
        ("p", "Generate subtitles from the audio itself (Tools \u2192 Docflix Whisper Transcriber), using faster-whisper or WhisperX."),
        ("bullet", "Batch processing with drag-and-drop"),
        ("bullet", "Translation to English"),
        ("bullet", "Word-level timestamps"),
        ("bullet", "Subtitle preview before saving"),
        ("p", ""),
        ("h3", "Sub Extractor"),
        ("p", "Batch-extract subtitle tracks already embedded in your files (Tools \u2192 Docflix Sub Extractor)."),
        ("bullet", "Probes each video for its embedded subtitle streams"),
        ("bullet", "Choose which types to pull: Main, Forced, SDH, CC, or all English"),
        ("h4", "Per-file Main / SDH / Forced columns"),
        ("p", "The file list shows one column per subtitle type, so you can see what each file actually contains before extracting anything:"),
        ("table_header", "Symbol|Meaning"),
        ("table_row", "—|This file has no track of that type"),
        ("table_row", "☐|It has one, and it will NOT be extracted"),
        ("table_row", "☑|It has one, and it WILL be extracted"),
        ("p", ""),
        ("bullet", "Click a cell to toggle that type for that one file"),
        ("bullet", "Click one cell, then SHIFT-CLICK another in the same column to apply the same setting to every file in between — files without that type are skipped, not forced"),
        ("bullet", "The Main / SDH / Forced checkboxes at the top are bulk sets: ticking one applies it to every loaded file"),
        ("note", "Many releases ship SDH as the only full English subtitle, with no plain English track at all. The columns make that visible before you extract — if Main shows an em-dash and SDH is ticked, SDH is your English subtitle. Post-Extraction Filters → Remove HI will strip the [sound cues] out of it."),
        ("p", ""),
        ("h4", "Post-Extraction Filters"),
        ("p", "The Filters button opens the same filter set as Batch Filter (Remove HI, tags, ads, duplicates, and the rest). They run on each subtitle as it is extracted, so a clean-up pass does not need a second trip through Batch Filter."),
        ("bullet", "Writes SRT, ASS or WebVTT alongside the source video"),
        ("bullet", "Optional OCR for bitmap (PGS / VobSub) tracks"),
        ("bullet", "Optional parallel extraction across CPU cores"),
        ("p", ""),
        ("h3", "Trailer Grabber"),
        ("p", "Fetch a title's official trailer from TMDB \u2014 matched by ID, so it is guaranteed to be the right one (Tools \u2192 Docflix Trailer Grabber, or the docflix-trailer command)."),
        ("bullet", "Downloads with yt-dlp, remuxes to MKV, and strips the YouTube tags"),
        ("note", "Bring your own yt-dlp. The Suite ships nothing and links nowhere \u2014 it auto-detects yt-dlp on your PATH, and if it is not there you install it yourself and point the tool at the binary."),
        ("p", ""),
        ("h3", "Media Renamer"),
        ("p", "Batch rename TV/movie files using TVDB/TMDB (Tools \u2192 Docflix Media Renamer):"),
        ...
        ("bullet", "Configurable filename template with flat and folder presets"),
        ("p", ""),
        ("h4", "Folder Templates"),
        ("p", "Use / in the filename template to automatically create folder hierarchies during rename:"),
        ("bullet", "{show} S{season}E{episode} {title} \u2192 Breaking Bad S01E01 Pilot.mkv"),
        ("bullet", "{show}/Season {season}/{show} S{season}E{episode} {title} \u2192 Breaking Bad/Season 01/Breaking Bad S01E01 Pilot.mkv"),
        ("bullet", "{show}/S{season}/{show} S{season}E{episode} {title} \u2192 Breaking Bad/S01/Breaking Bad S01E01 Pilot.mkv"),
        ("p", "Folders are created relative to the source file's location. Undo will move files back and clean up any empty folders that were created."),
        ("p", ""),
        ...
        ("p", ""),
        ("h3", "Media Rescale"),
        ("p", "Batch resize video files (Tools \u2192 Docflix Media Rescale, Ctrl+Shift+R). Also available as the docflix-scale standalone command."),
        ("p", ""),
        ("h4", "Resolution Presets"),
        ("table_header", "Preset|Dimensions"),
        ("table_row", "Original|No scaling (passthrough)"),
        ("table_row", "2160p (4K)|3840x2160"),
        ("table_row", "1440p (2K)|2560x1440"),
        ("table_row", "1080p|1920x1080"),
        ("table_row", "720p|1280x720"),
        ("table_row", "480p|854x480"),
        ("table_row", "Custom|User enters WxH"),
        ("p", ""),
        ("h4", "Features"),
        ("bullet", "GPU-accelerated scaling (NVENC, QSV, VAAPI) \u2014 GPU handles encoding, scaling adapts per backend"),
        ("bullet", "Aspect ratio preservation \u2014 width auto-calculated from actual decoded content dimensions"),
        ("bullet", "Upscale warning \u2014 flags files where target resolution exceeds source"),
        ("bullet", "Smart probing \u2014 extracts a frame at 30% of duration to detect actual content size (handles letterboxed and variable-resolution files)"),
        ("bullet", "Encoder selection with preset and CRF controls"),
        ("bullet", "Audio passthrough (copy) or re-encode"),
        ("bullet", "Real-time progress bar with ETA"),
        ("bullet", "Drag-and-drop file support"),
        ("bullet", "All settings saved to preferences between sessions"),
        ("p", ""),
        ("h3", "Test Encode"),
    ...
    ("Keyboard Shortcuts", [
        ...
        ("table_row", "Ctrl+Shift+R|Media Rescale"),
    ...
    ("Preferences", [
        ...
        ("bullet", "Media Renamer API keys, provider, template"),
        ("bullet", "Media Processor settings (all options)"),
        ("bullet", "Media Rescale settings (resolution, encoder, preset, CRF, output folder)"),
        ("p", ""),
...
def show_manual(app):
    ...
    def _render_all():
        ...
        # Ensure text selection via mouse works — explicitly bind
        # mouse events so the PanedWindow doesn't steal them
        def _on_click(event):
            text.focus_set()
