...
2. **Subtitle Editor CC awareness** — The standalone Subtitle Editor now detects EIA-608/CEA-708 closed captions when a video file is dragged in or opened. CC detection uses `detect_closed_captions()` from gpu.py (ffprobe ATSC A53 side data scan). If CC is found, it appears as a virtual track in the stream picker alongside regular subtitle streams (labeled "CC" with codec `eia_608`). Extraction uses `extract_closed_captions_to_srt()` (ffmpeg lavfi movie[subcc] filter with automatic artifact cleanup). When saving CC-sourced edits, the editor writes a standalone `.srt` file alongside the video (CC can't be remuxed back as closed captions). The internal queue editor (`show_subtitle_editor`) is unchanged — CC detection there is handled by the conversion queue's own file probing.
3. **Separate EIA-608/CEA-708 detection and extraction** — New `detect_cc_types()` function in gpu.py uses ccextractor's report mode to distinguish EIA-608 from CEA-708 (falls back to ffprobe if ccextractor unavailable). The subtitle editor now creates separate virtual tracks for each CC type found (labeled "Closed Captions (EIA-608)" and "Closed Captions (CEA-708)"). Extraction uses ffmpeg lavfi `movie[subcc]` for 608 and `ccextractor --svc 1` for 708. The `extract_closed_captions_to_srt()` function gained a `cc_type` keyword parameter (default `'eia_608'`) — all existing callers are unaffected. Updated both gpu.py and the duplicate in video_converter.py.
4. **Override Settings dialogs resizable** — Both the main app and Media Processor override settings dialogs were locked at fixed size (`resizable(False, False)`) and couldn't be minimized or maximized. Changed to `resizable(True, True)` with `minsize()` constraints. Removed `transient()` which was causing the window manager to strip minimize/maximize buttons on Linux/X11.
5. **Whisper Transcriber auto-install** — When neither faster-whisper nor WhisperX is installed, the transcriber now prompts the user to install one via a `Yes/No/Cancel` dialog (Yes=faster-whisper, No=WhisperX, Cancel=close). Installation runs `pip install --user --break-system-packages` in a background thread with a modal progress dialog showing live pip output, an indeterminate progress bar, and a status label tracking milestones. On success the dialog auto-closes after 1.5s; on failure it stays open with a Close button so the user can read the error. The same install prompt appears when the user switches to an unavailable backend in the dropdown, or clicks Extract with no backend available. Backend availability hints in the settings panel refresh dynamically after each install.
6. **TV Renamer scroll handler fix** — The show picker dialog's `bind_all` scroll handlers (Button-4, Button-5, MouseWheel) persisted after the dialog was closed via the Load or No Match buttons, because those paths called `dlg.destroy()` directly without unbinding. This caused a flood of `TclError: invalid command name` errors whenever the user scrolled in any other window. Fixed by routing all close paths (`_ok`, `_no_match`) through `_on_close()` which unbinds first. Added try/except guards to all scroll handlers in both the show picker and template settings dialogs as a safety net.
7. **Renamed Sub Ripper to Sub Extractor** — All user-facing text (window title, menu label, dialog titles, log messages) changed from "Docflix Sub Ripper" to "Docflix Sub Extractor". Internal file name, function names, and preference keys unchanged.
8. **Whisper Transcriber max line width default** — Changed the "Max width" setting default from 0 (off) to 42 characters (broadcast/TV industry standard). The line wrapping infrastructure was already in place via `apply_line_wrap()` — only the default value was changed.
9. **Whisper Transcriber 2-line cue cap** — `apply_line_wrap()` now caps each subtitle cue at 2 lines (configurable via `max_lines` parameter). Segments that exceed 2 lines after wrapping at 42 characters are split into multiple cues with timing distributed proportionally by character count. Previously, long segments could produce 3-4+ lines per cue.
10. **GPU encoder combobox desync fix** — The encoder combobox displayed the GPU name (e.g. "RTX 2000E Ada Generation (NVENC)") but the underlying `encoder_mode` StringVar held `'cpu'` after loading saved preferences. Root cause: `load_preferences()` updated the StringVar but never called `on_encoder_change()` to sync the combobox and dependent UI controls. This caused CPU encoding despite the UI showing GPU selected. Fixed by adding `on_encoder_change(silent=True)` and other UI sync calls at the end of `load_preferences()`.
